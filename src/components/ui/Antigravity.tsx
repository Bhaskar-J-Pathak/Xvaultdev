...
const AntigravityInner: React.FC<AntigravityProps> = ({
  ...
  // Build a 1×128 gradient canvas texture when gradient props are provided
  const gradientTexture = useMemo(() => {
    ...
    // top of capsule → gradientEnd (darker/richer), bottom → gradientStart (lighter)
    gradient.addColorStop(0, gradientEnd ?? color);
    gradient.addColorStop(1, gradientStart ?? color);
  ...
  return (
    <instancedMesh ref={meshRef} args={[undefined, undefined, count]}>
      ...
      {gradientTexture ? (
        <meshBasicMaterial map={gradientTexture} transparent opacity={0.82} />
      ) : (
